...
if [[ $# -ne 4 ]]; then
  ...
  echo "modes: seq768 | seq512 | s384_low_lr" >&2
  exit 2
fi
...

{
  ...
  elif [[ "$MODE" == "s384_low_lr" ]]; then
    "$PY" experiments/pretrain/scripts/train_pretrain_optimized.py \
      --save_dir "$OUT/02_s384_lr1e5" \
      --save_weight "${RUN_NAME}_s384_lr1e5" \
      --init_weight "$STAGE1" \
      --data_path experiments/pretrain/strict_splits/pretrain_strict_train.jsonl \
      --tokenizer_path model \
      --epochs 1 \
      ...
      --fused_adamw \
      --stream_packed
    eval_final "$OUT/02_s384_lr1e5/${RUN_NAME}_s384_lr1e5_768.pth" 384
