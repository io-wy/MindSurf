# 外部锚点评测：移植成立，锚点未成立（2026-07-22）

**结论：外部锚点无法用任何当前已发布的上游 checkpoint 建立。** 原因不是"做不动"，
而是一个可量化的事实：上游发布的权重与本项目用的**不是同一个 tokenizer**，
per-token loss 因此不可比。模型移植本身已证明逐位忠实——正是这个忠实的移植让
未成立的原因可以被测出来，而不是被猜出来。
...
与本移植在同一组随机 token id 上比对 logits（batch 2 × seq 128，float32，
seed 20260721）：
...
一个仪器**，需要自己的效度验证与噪声底，不是本项目现成的标尺。

## 4. 处置

- **不报告任何方向的比较。** 候选与锚点之间既不声称优也不声称劣。
- **锚点标注为未成立，不以其他支柱代偿。** 发布说明中所有结论都相对本项目自己的
  历史最优参照，属内部纵向比较，其局限已在发布说明明示。
- **移植与闸门保留**（`scripts/evaluate_external_anchor.py`，9 项测试）。上游若
  发布基于当前 tokenizer 训练的权重，锚点就是一条命令的事。
- 未确立的事项如实记录：因为没有做任何比较，**未确认**噪声底是否适用于锚点式
  比较。
